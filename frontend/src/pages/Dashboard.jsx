...
import { streamAPI, destinationAPI, videoAPI } from '../services/api';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Source" icon={<FiRadio size={16} className="text-gray-500" />}>
          {streamStatus.running && activeStream?.sourceUrls?.length > 0 ? (
            <>
              <div className="text-lg font-bold text-white truncate">
                URL {Math.min(streamStatus.currentIndex + 1, activeStream.sourceUrls.length)} of {activeStream.sourceUrls.length}
              </div>
              <div className="text-xs text-gray-500 mt-1 capitalize">{activeStream.sourceType}</div>
            </>
          ) : streamStatus.running && activeStream?.playlist?.length > 0 ? (
            <>
              <div className="text-lg font-bold text-white truncate">
                {activeStream.playlist[streamStatus.currentIndex || 0]?.originalName || activeStream.sourceType}
              </div>
              <div className="text-xs text-gray-500 mt-1">
                Now playing {streamStatus.currentIndex + 1} of {activeStream.playlist.length} · {activeStream.sourceType}
              </div>
            </>
          ) : (
            <>
