...
A production-ready, full-stack live streaming management platform for [stream.aayushbaral.com](https://stream.aayushbaral.com). Authenticated administrators can upload videos (automatically joined into a looping playlist), stream single or multiple YouTube / Facebook / TikTok URLs one by one, configure multiple RTMP destinations (YouTube, Facebook, custom), start/stop live streams with FFmpeg, view live HLS previews, and overlay a channel logo, a scrolling news ticker and a live clock onto the video — all from a modern dark broadcast-style dashboard.

## Features

- JWT-authenticated admin dashboard (login / register)
- Video upload with progress bar — every upload is **automatically added** to the active playlist (persistent storage at `/data/videos`)
- Playlists play **one video at a time, in order** (with optional loop)
- **YouTube / Facebook / TikTok** sources (video **or** live URLs) — add one or many URLs and play them one by one; resolved with yt-dlp and broadcast live
...
- The currently-playing video/URL is highlighted in **green** in the Upload page
- Channel logo, breaking-news scroller and a live **HH:MM:SS clock** burned into the video output (clock renders in `Asia/Kathmandu` by default)
...
The stream source is chosen in the **Upload** page under *Stream Source*, and the uploaded queue is managed on the **Playlist** menu page.

The **Playlist** page shows every uploaded video with its duration and file size, lets you build the queue in click order, and displays a **Now Playing** panel beside it (LIVE/OFFLINE status, current item, position in queue, uptime, loop toggle, and quick Start/Stop — broadcasting to the HLS preview; pick RTMP destinations on the Upload page).

1. **Upload Video** — pick one or more uploaded files in click order to build a playlist. Uploads are added to the playlist automatically. The playlist plays through one video at a time and, when enabled, loops back to the start.
2. **YouTube** — paste one or more video **or live-stream** links. Multiple links play one by one.
3. **Facebook** — same as YouTube (video or live links).
4. **TikTok** — same as YouTube (video or live links).

URL sources are resolved with `yt-dlp` into directly playable streams. Each link is normalized (same size/codec) when needed, and VOD links in a playlist continue one by one. A **live / undetermined** source inside a playlist must be streamed individually (it never ends, so it cannot be chained).

The currently-playing video or URL shows a green border and **LIVE NOW** badge. On the dashboard, the Source card shows "Now playing X of Y".
...
| `STREAM_TIMEZONE` | IANA timezone for the clock burned into the video (default `Asia/Kathmandu`) |
| `YTDLP_PATH` | Optional full path to a `yt-dlp` binary (otherwise `python -m yt_dlp` or bundled exe is used) |
| `TICKER_FONT` | Optional path to a font file for the on-video ticker/clock |
...
| POST | `/api/streams/start` | Start stream — body: `sourceType` (`upload`/`youtube`/`facebook`/`tiktok`), `videoIds: []` for upload playlists, `sourceUrls: []` for URL playlists, `loop`, `destinationIds` |
